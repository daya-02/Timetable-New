...
class TimetableExcelService:
    ...
    def _build_semester_sheet(self, ws, semester: Semester, grid: Dict, allocations: List):
        ...
        if semester.departments:
            dept_name = "DEPARTMENT OF " + semester.departments[0].name.upper()
        ...
        subject_data = {}
        for alloc in allocations:
            key = (alloc.subject_id, alloc.teacher_id, alloc.component_type)
            ...
            subject_data[key]["hours"] += 2 if alloc.component_type.value == "lab" else 1

        curr_row = start_row_sub + 1
        for data in subject_data.values():
            subj = data["subject"]
            tname = data["teacher"].name if data["teacher"] else "***"
            hours = data["hours"]
            ...
            dept = semester.departments[0].code if semester.departments else "AI"
    ...
    def generate_semester_excel(self, semester_id: int) -> BytesIO:
        semester = self.db.query(Semester).options(
            joinedload(Semester.departments)
