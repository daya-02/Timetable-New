...
class TimetableGenerator:
            ...
            for comp_label, comp_reqs in reqs_by_component.items():
                ...
                if missing_classes:
                    for mc in missing_classes:
                        fail_msg = (
                            f"[MISSING CLASS] Elective Group '{group_name}' (Year {year}, Basket {basket_id}): "
                            f"Class {mc} has no mapped requirements for component '{comp_label}'. "
                            f"Generation must fail."
                        )
                        print(f"        {fail_msg}")
    ...
    def _schedule_labs_readonly(
        ...
        for req in sorted(lab_reqs, key=lambda r: r.hours_per_week, reverse=True):
            ...
            # Track teacher allocations for load balancing across blocks
            ...
            for day, (start_slot, end_slot) in lab_slots:
                ...
                # NEW: Select the next teacher from pool using load balancing (not all teachers)
                ...
                if not selected_teacher:
                    rejection_reasons.append(f"Day {day} {start_slot}-{end_slot}: No eligible teacher available")
                    continue
                ...
                # Schedule ONLY the selected teacher (rotated) into the room and slots
                for idx, slot in enumerate([start_slot, end_slot]):
                    entry = AllocationEntry(
